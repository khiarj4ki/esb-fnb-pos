 ...
 */
class PrintEndShift extends Model {
    ...
    public function rules() {
        return [
            [['shiftDetailID', 'stationID'], 'required'],
            [['shiftDetailID', 'stationID'], 'integer'],
            [['reprint'], 'boolean'],
            [['shiftDetailID'], 'validateShift'],
    ...
    public function validateShift($attribute) {
        ...
        $this->shiftLogDetailModel = ShiftLogDetail::find()
            ->joinWith('shiftLog')
            ...
            ->one();
        if (!$this->shiftLogDetailModel) {
            $this->addError($attribute, 'Invalid shift detail ID');
        } else {
            $this->endDate = $this->shiftLogDetailModel->shiftTime;
            $previousShiftDetailModel = ShiftLogDetail::find()
                ->joinWith('shiftLog')
                ->andWhere(['branchID' => $branchID])
                ->andWhere([ShiftLogDetail::tableName() . '.shiftID' => $this->shiftLogDetailModel->shiftID])
                ->andWhere(['<', 'shiftTime', $this->shiftLogDetailModel->shiftTime])
                ->orderBy('shiftTime DESC')
                ->one();
            if ($previousShiftDetailModel) {
                $this->startDate = $previousShiftDetailModel->shiftTime;
            } else {
                $shiftModel = ShiftLog::find()
                    ->andWhere(['branchID' => $branchID])
                    ->andWhere(['shiftID' => $this->shiftLogDetailModel->shiftID])
                    ->one();
                $this->startDate = $shiftModel->shiftInTime;
            }
        }
    }
    ...
    public function doPrint() {
        ...
        try {
            ...
            $shiftReportModel->scenario = ShiftReport::SCENARIO_BY_TIME;
    ...
    private function printPageHeader($data) {
        ...
        $printer->text(Yii::t('app', 'CASHIER SHIFT REPORT'));
    ...
    private function printShiftSummary($data) {
        ...
        $printer->text('  |  '.date_format(date_create($this->shiftLogDetailModel->shiftLog->shiftInTime),'d-m-Y'));
        if ($this->stationModel->printerTypeID == '4' || $this->stationModel->printerTypeID == 15) {
            $printer->getPrintConnector()->write("\x0A");
        } else {
            $printer->feed(1);
...
//        $shiftLogDetailModel = ShiftLogDetail::find()
//            ->with('shiftLog.branch')
//            ->with('shiftUser')
//            ->andWhere(['ID' => $this->shiftDetailID])
//            ->one();
        ...
        return [
            'branchName' => $this->shiftLogDetailModel->shiftLog->branch->branchName,
            'cashierName' => $this->shiftLogDetailModel->shiftUser->fullName,
