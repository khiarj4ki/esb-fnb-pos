...
class ShiftCashReport extends Model {
    ...
    public function getSalesRecap() {
        ...
        $shiftLogCashQuery = (new Query())
            ->select([
                'openingBalance' => 'COALESCE(startingCash, 0)',
                'closingBalance' => 'COALESCE(endingCash, 0)'
            ])
            ->from(ShiftLogCash::tableName())
            ->andWhere(['>=', 'shiftInTime', $this->startDate])
            ->andFilterWhere(['<=', 'shiftOutTime', $this->endDate])
            ->one();

        $cashPaymentQuery = (new Query())
            ->select([
                'cashSales' => 'COALESCE(a.paymentAmount, 0)'
            ])
        ...
        $shiftData = [];
        if ($this->shiftLogModel && $this->shiftLogModel->shiftLogCash) {
            $shiftDataDetail = [];
            foreach ($this->shiftLogModel->shiftLogCash as $shiftLogCash) {
                $paymentTotal = 0;
                $shiftTitle = "Shift " . $shiftLogCash->shiftNumber;
                $shiftSales = (new Query())
                    ->select(['paymentTotal' => 'COALESCE(paymentTotal, 0)'])
                    ->from(SalesHead::tableName() . ' a')
                    ->leftJoin(['b' => SalesPlatformFee::tableName()],
                                "a.salesNum = b.salesNum" . " AND b.platformFeeTypeID = 1")
                    ->andWhere(['branchID' => $this->branchID])
                    ->andWhere(['>=', 'salesDateOut', $shiftLogCash->shiftInTime])
                    ->andFilterWhere(['<', 'salesDateOut', $shiftLogCash->shiftOutTime])
                    ->andWhere(['statusID' => 8])
                    ->andWhere(['NOT IN', 'a.salesNum', SalesPayment::getNonSalesQuery($this->branchID,
                            $shiftLogCash->shiftInTime, $shiftLogCash->shiftOutTime)])
                    ->all();

                if ($shiftSales) {
                    foreach ($shiftSales as $sales) {
                        $paymentTotal += (float) $sales['paymentTotal'];
                    }
                }
                $shiftDataDetail[$shiftTitle] = $paymentTotal;
            }
            $shiftData['shiftData'] = $shiftDataDetail;
        }

        return array_merge([
            'pendingSales' => (float) $pendingSales['grossSales']
            ], $closedSales, $cancelSales, $voidSales, $shiftLogCashQuery, $cashPaymentQuery, $shiftData);
    }
    ...
    public function getSalesPaymentPerPaymentMethod() {
        ...
        $salesPaymentMethods = (new Query())
            ...
            ->orderBy('c.paymentMethodName ASC, a.salesNum')
            ->all();
