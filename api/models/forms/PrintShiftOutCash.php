 ...
 */
class PrintShiftOutCash extends Model {
    public $shiftID;
    public $stationID;
    ...
    public function validateShift($attribute) {
        ...
        if (!$this->shiftLogModel) {
            ...
            if (!$this->shiftLogModel->shiftOutTime) {
                ...
                $this->shiftLogCashModel = $this->shiftLogModel->shiftLogCash;
            }
        }
    }
    ...
    public function doPrint() {
        ...
        try {
            ...
            $shiftReportModel->scenario = ShiftCashReport::SCENARIO_BY_ID;
    ...
    private function printPageHeader() {
        ...
        $printer->text(Yii::t('app', 'END DAY REPORT') . $this->tempString);
        ...
        $printer->text(str_pad(Yii::t('app', 'Start Day Date'), 18, ' '));
        $printer->text(' : ');
        ...
        $printer->text(str_pad(Yii::t('app', 'End Day Date'), 18, ' '));
        $printer->text(' : ');
    ...
    private function printShiftSummary($data) {
        ...
        if ($data['shiftData']) {
            $printer->text(str_pad(Yii::t('app', 'Shift Report'), $charLength - 15, ' '));
            $this->printLineBreak();
            foreach ($data['shiftData'] as $title => $total) {
                $printer->text(str_pad($title, $charLength - 15, ' '));
                $printer->text(' : ');
                $printer->text(str_pad(number_format($total, $salesDecimalSetting, "$salesDecimalSeparatorSetting", "$reverseDecimalSeparator"),
                        12, ' ', STR_PAD_LEFT));
                $this->printLineBreak();
            }
        }
